...
任何一支 RST 打不開 (GPIO busy、權限) 時整輪直接失敗並指名腳位，不會只掃其他六顆：
那支腳沒被拉低時可能是高的 (GPIO4 開機預設上拉)，晶片醒著搶 MISO，其他六顆全都會讀不到，
繼續掃只會回報六條線「線路異常」，錯的方向。

### 持續背景輪巡與黏性合併

首頁按「開始」、印表機檢查通過後，`RFIDReaderProvider.startPolling()` 開始在背景一輪接一輪掃
(每輪之間休息 `pollGapMs`)，直到訂單頁按「分析」、回到首頁或 app 關閉才停 (`stopPolling()`)。
首頁只等第一輪完成 (上限是整輪逾時) 就進到下一頁，不再固定等 3 秒。
托盤晚放、中途換餐、單次漏讀都不需要客人按「重新感應」。

每輪的結果做黏性合併：

- 這一輪讀到卡片：直接更新；讀到不同的 UID 立刻換掉 (中途換餐)。
- 這一輪沒讀到 (或線路異常)：距離上次讀到不超過 `stickyRounds` 輪就保留上次的卡片，
  超過才清掉，偶發漏讀不會讓餐點閃一下消失。
- 線路異常要連續 3 輪都異常才會在訂單頁顯示「第 N 號感應區暫時無法使用，請叫服務人員」
  (對客人說的話，字級跟頁面其他文字一致)。訂單頁顯示的是合併後的狀態，不會每輪閃 spinner。

「重新感應」按鈕會清掉保留的卡片並立刻重讀一次 (單次掃描，結果直接取代)。
分析與列印期間不輪巡，避免搶 CPU。連線檢測與自動最佳化開始前會先停下輪巡並等目前這一輪結束，
做完再恢復；背景輪巡與單次掃描也會互相等待，不會同時碰硬體。

### 預設值與預期時間

預設值刻意保守：實測經驗是線長需要更長等待，而 `ranges` 的上限允許回到舊值，
所以先用長線第一次開機就能用的值起步，上機後再用「自動最佳化」把每顆縮到最小可靠值：
...
| `antennaSettleMs` | 200 | 是 | ISO 14443-3 只要求卡片在場開啟後 5 ms 內就緒；200 是長線的保守起點 (舊版 500)，最佳化會往下縮 |
| `reqaTimeoutMs` | 25 | 是 | Arduino library 的 timer 設定 |
| `reqaAttempts` | 3 | 是 | 卡片剛上電偶爾會漏掉第一次 REQA；有卡時第一次通常就中，只有沒卡時才會全部試完 |
| `commDeadlineMs` | 50 | 是 | 至少 `reqaTimeoutMs + 25`，留給 isolate 被排程回來的時間 |
| `interReaderGapMs` | 200 | 否 | 兩顆之間的間隔 (最後一顆之後不等)；保守起點 (舊版 500)，最佳化不掃這一項，要縮請手動調 |
| `postScanSettleMs` | 0 | 否 | 舊版為 500；背景輪巡的休息改用 `pollGapMs` |
| `scanTimeoutSec` | 10 | 否 | 整輪逾時，會自動至少是最壞情況的兩倍 (保守預設值下約 15 秒) |
| `pollGapMs` | 300 | 否 | 持續背景輪巡時每輪之間的休息時間 |
| `stickyRounds` | 3 | 否 | 沒讀到時先保留上次卡片幾輪 |
| `enabledReaders` | `[]` (全部) | — | 實際有接線的讀卡機編號，例如 `[1, 2]`。沒列的不掃描、不顯示卡片、不算線路異常，但 RST 仍拉低 |
...
| 設定 | 七顆都有卡 | 七顆都沒卡 |
|---|---|---|
| 保守預設值 50 / 200 / 25 ms × 3，顆間 200 | 約 3.0 s | 約 3.5 s |
| 最佳化後例如 15 / 2 / 10 ms × 3，顆間手動調成 5 | 約 0.2 s | 約 0.4 s |

顆間間隔不在最佳化的搜尋範圍 (理由見第 5 節)，最佳化之後想把一輪縮到零點幾秒，
要自己在設定頁把 `interReaderGapMs` 調小。
...
- 環境變數的值不是整數時會被忽略，設定頁的「設定來源」會列出被忽略的變數名稱。
- `readers` 的 key 會正規化，寫 `"7"` 跟 `"07"` 一樣。

### 只接了部分讀卡機

開發或測試時常常只接一顆或兩顆。七顆共用 SPI bus，只靠 RST 分辨誰在講話：
沒接的位置在掃描時 bus 上沒有晶片醒著，會被判成「線路異常」；更麻煩的是，
接上的那顆若 RST 沒有被 Pi 真正拉低 (線鬆脫、插到隔壁的 3.3V 腳、用實體腳位編號接錯)，
它會回應每一個位置的讀取，七個位置都顯示同一道菜。

`enabledReaders` 告訴程式哪幾顆真的有接：

```json
{ "enabledReaders": [1] }
```

- 沒列的讀卡機不掃描，訂單頁與設定頁都不會把它們當成沒卡或線路異常；RST 仍然會拉低。
- 設定頁「編輯設定」裡有勾選框，也可以用環境變數 `RFID_ENABLED_READERS=1,2` 或
  CLI `set enabledReaders=1,2` (全部：`set enabledReaders=all`) 修改。
- 空清單或不寫代表全部七顆。

不論有沒有設定這一項，同一輪裡多顆讀卡機讀到相同 UID 時，app 只算一次
(保留編號最小的那顆)：一張卡不會同時在兩個感應區，這幾乎都是 RST 沒拉低造成的。
被忽略的讀卡機會在設定頁的讀卡機列表標成橘色「與讀卡機 XX 是同一張卡」，
並在列表上方提示檢查接線。
...
設定頁左欄的「輪巡時序設定」卡片預設收合 (1280×720 時展開才佔空間，讀卡機列表不會被擠掉)，
標題列顯示目前的 SPI 時脈與主要等待值；展開後顯示有幾顆有個別覆寫、估計一輪時間
(全部沒卡 / 啟用的都有卡)、上次實測耗時、設定來源 (設定檔壞掉時紅字)、四個按鈕，
以及收在「詳細診斷」裡的工程師資訊 (每個欄位的值、每顆讀卡機的摘要如 `重寫×N`)。
讀卡機列表本身只顯示卡片、餐點與錯誤訊息。

| 按鈕 | 做什麼 |
|---|---|
| 編輯設定 | 每個全域值用「− 值 +」按鈕調 (觸控 kiosk 沒有鍵盤也能用，長按連續加減，範圍自動夾住)，列出每顆的覆寫值並可清除，按「儲存」寫入設定檔並立即生效 |
...
2. **確認階段**：每顆從「起點值」跑 N 輪。起點 = max(目前生效的值, 一般候選值的最大值)，
   ...
   讀不到卡的讀卡機把參與搜尋的參數一起往上放寬再確認：RST 100 → 200 → 500 ms、
   天線 50 → 100 → 200 ms、REQA 逾時 50 → 100 ms (勾了「也掃 REQA 次數」時次數 3 → 4)；
   「需要更長等待」的解因此也找得到。放寬到底仍讀不到才標為「不穩定」，
   不參與後面的搜尋並維持原設定，通常是卡片沒放好或線路問題。
3. **掃描階段**：依序對 `rstSettleMs`、`antennaSettleMs`、`reqaTimeoutMs`
   (預設不掃 `reqaAttempts`，見下) 由起點往小試。候選階梯是升階值與一般候選值的聯集，
   所以天線從預設 200 起步時會經過 100、50 再到 20 → 10 → 5 → 2 → 0，
   RST 是 50 → 30 → 20 → 15 → 10，REQA 逾時是 25 → 15 → 10 → 5。
   每顆讀卡機各自有自己的候選值與進度，但一輪掃描本來就會輪過七顆，
   所以七顆在同一輪裡各測各的候選值，每顆各自搜尋不需要七倍時間。
   每個候選值跑 N 輪，全中才往下一個更小的值走；
   取最小可過的值之後再往上加「安全餘裕」階數 (預設 1 階，不超過起點)。
4. **驗證階段**：用最終值跑 M 輪。任何一顆漏讀就把它參與搜尋的參數各放寬一階，
   然後重驗 (最多 3 次)；仍失敗的讀卡機退回原設定並標記。取消時所有讀卡機都標為未驗證。
...
預設 N = 8、M = 60、放寬重驗最多 3 次：5 輪全中在真實漏讀率 20% 時仍有 33% 的機率通過，
8 輪把它壓到 17%，60 輪驗證再把邊緣值篩掉。保守預設值下一輪 3 秒多、越掃越快，
整個流程約 3 到 6 分鐘；有讀卡機需要放寬時會更久。輪數與餘裕都可以在對話框裡調。

`reqaAttempts` 預設不掃 (對話框可勾「也掃 REQA 次數」，CLI 加 `--sweep-attempts`)：
它只在沒卡時花時間，有卡時 REQA 第一次通常就中，把它砍到 1 只省空讀卡機的時間，
卻少一次重試的保險，而 N 輪全中的統計抓不到幾 % 的漏讀率。

「穩定」的判定仍是統計上的，卡片位置、溫度改變後可以重跑一次。
...
dart run scripts/rfid_calibrate.dart optimize --sweep-rounds 12 --verify-rounds 100 --margin 2
dart run scripts/rfid_calibrate.dart optimize --sweep-attempts        # 連 reqaAttempts 也掃
...
bus 上不會有別顆醒著)；沒給 `--readers` 時，設定檔的 `enabledReaders` 有值就只測那幾顆。
所有指令都可以加 `--file <path>` 指定設定檔、`--verbose` 看細節。
...
| 只放一張卡，卻每個位置都顯示同一道菜 (設定頁標「與讀卡機 XX 是同一張卡」) | 放卡那顆的 RST 沒被 Pi 拉低：線鬆脫、插到隔壁的 3.3V 腳、用實體腳位編號接錯，醒著的晶片回應每一個位置 | 掃其他顆時量那顆的 RST 腳應為 0V；用 `bench --readers 2` 驗證 (1 號 RST 拉低後 2 號仍讀到卡就是接線問題)。訂單頁已只算一次；只接部分讀卡機時把沒接的在設定頁取消勾選 (`enabledReaders`) |
| 沒接的位置一直「線路異常」 | 那幾顆本來就沒接 | 設定頁「編輯設定」把沒接的取消勾選，或 `set enabledReaders=1,2` |
...
| 整輪錯誤，訊息說「讀卡機 N 的 RST GPIO 打不開」 | 那支腳被別的程式佔住 (CLI 與 app 同時跑)、權限 | 關掉另一個程式；這時不會只掃其他六顆，因為沒拉低的 RST 會讓其他六顆全部讀不到 |
...
這是不動接線時最接近「硬體修正」的手段。pad 驅動力只影響 Pi 輸出的 SCK、MOSI 與 RST；
MISO 是 RC522 在驅動，它的邊緣取決於模組與線，Pi 這邊調不到：

```bash
sudo apt install pigpio                      # 只需一次
sudo ./scripts/gpio_drive_strength.sh        # 看目前的值 (不帶參數只顯示，不會改)
...
| `lib/provider/rfid_reader_provider.dart` | 背景輪巡、黏性合併、單次掃描與校正的互斥 |
